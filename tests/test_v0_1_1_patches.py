...
class TestB4ServerInfoListsAccountNumbers:
    ...
    def test_b4_tool_count_is_fifteen(self) -> None:
        info = meta.get_server_info_impl()
        assert len(info["tools"]) == 15, (
            "Tool-count regression — server_info should report 15 tools including get_twrr_analysis."
        )
    ...
    def test_b4_readme_zh_advertises_fourteen_tools(self) -> None:
        ...
        assert "工具列表（15 个）" in body or "15 个）" in body  # noqa: RUF001 (matching README_zh literal)
